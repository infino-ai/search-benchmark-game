...
( cd "$SBG/engines/infino-branch" && cargo build --release --bin build_index --bin do_query )
( cd "$SBG/engines/infino-main"   && cargo build --release --bin build_index --bin do_query )

# Build one index per codec.
"$SBG/engines/infino-branch/target/release/build_index" "$SBG/idx256" < corpus.json
...
for m in top10 top100 top1000 count; do
  ...
  profile_one infino-branch "$SBG/idx256" "branch256_$m" "/tmp/$m.txt"
done
